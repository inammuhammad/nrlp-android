...
import com.onelink.nrlp.android.databinding.FragmentRedemptionOpfVoucherBinding
...
import com.onelink.nrlp.android.features.redeem.viewmodels.RedemptionOPFVoucherViewModel
...
class RedemptionOPFVoucherFragment : BaseFragment<RedemptionOPFVoucherViewModel,FragmentRedemptionOpfVoucherBinding>
        (RedemptionOPFVoucherViewModel::class.java) , OneLinkAlertDialogsFragment.OneLinkAlertDialogListeners {
    ...
    companion object {
        @JvmStatic
        fun newInstance() = RedemptionOPFVoucherFragment()
    }

    override fun getTitle(): String = resources.getString(R.string.redeem)
    override fun getLayoutRes() = R.layout.fragment_redemption_opf_voucher
    override fun getViewM(): RedemptionOPFVoucherViewModel =
        ViewModelProvider(this,viewModelFactory).get(RedemptionOPFVoucherViewModel::class.java)
    ...
    override fun init(savedInstanceState: Bundle?) {
        ...
        binding.btnPositive.setOnClickListener {
            if (viewModel.checkVoucherValidation(binding.etVoucher.text.toString())) {
                ...
                viewModel.makeInitializeRedemptionCall(redeemPartnerModel.partnerName,
                    ...
                    "",
                    binding.etVoucher.text.toString())
            } else {
                binding.tilVoucher.error = "Please enter valid Voucher No"
            }
        }
            ...
            when (response.status) {
                Status.SUCCESS -> {
                    ...
                    response.data?.let {
                        ...
                        val voucherNo = binding.etVoucher.text.toString()
        ...
        viewModel.observeInitializeRedemptionOTP().observe(this, Observer { response ->
            when (response.status) {
                Status.SUCCESS -> {
                    ...
                    response.data?.let {
                        redeemSharedViewModel?.setTransactionId(it.transactionId)
                        redeemSharedViewModel?.setPSID(binding.etVoucher.text.toString())
