...
class RedemptionOPFVoucherViewModel @Inject constructor(private val redPartnerRepo: RedemptionRepo) : BaseViewModel() {
    val voucherNo = MutableLiveData<String>("")
    ...
    fun checkVoucherValidation(string: String) : Boolean {
        return string.isNotEmpty() && ValidationUtils.isOPFPSIDLengthValid(string)
                && ValidationUtils.isAliasValid(string)
    }
